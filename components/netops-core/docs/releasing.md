...
is live and a reader is never sent to a download that no longer exists. Source history remains reachable
by commit on `main`; the original release artifacts and tag signature are not preserved by that
source history. A reader who needs an earlier source finds that
