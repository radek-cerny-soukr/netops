...
Enrollment moved to the current shape - `inventory.json` and `vault.json` at file version 2, with the separate `egress-policy.json` and `runner.json` - in the 0.3.0 line; see the top of [Configuration](configuration.md#the-four-operator-files) and the note at [Installation §3.6](installation.md#3-prepare-configuration-and-trust-on-the-proxy-host). A file still carrying `"version": 1`, or a leftover `target-policy.json`, is refused by name; there is no automatic migrator, and the preflight command will not silently reinterpret it either - it fails closed exactly like the proxy.
...
6. **Replace only after it passes**: move the validated copies onto the live configured paths (a same-filesystem `mv` is atomic), then run the preflight command again with no arguments to confirm the live files are the ones just validated. Only then restart the service, following the guarded sequence in [Installation §7](installation.md#7-start-the-service-only-after-the-check).
7. Once the new files are confirmed live and working, securely remove the old `version: 1` files and any leftover `target-policy.json`. Do not leave two live-looking copies of the vault on disk at once.
