...
`ssh_read` drives the OpenSSH client of `netops-core` directly; there is no vendor session driver between the catalogue and the device any more, so nothing but the reviewed command and the platform preamble is sent. Which of the two transport modes a platform uses is a table in the code, not a field of the configuration:
...
`extreme_exos` is the only platform with a preamble: `disable cli paging` is sent as its own command before the query, exactly as the auditor does it. FortiOS has no preamble at all, which is why it still requires the externally verified `output standard` before enrollment: the helper never changes a pager on a device. The wire test `tests/test_ssh_wire_safety.py` substitutes `ssh` and `ssh-keyscan` on `PATH` and pins, for every platform of the catalogue, the exact argument vector, the environment, the known-hosts line and the bytes sent, so a change that adds a command, drops `-F /dev/null`, weakens an option or lets a login transcript escape fails the release gate.
