...
def test_dependency_drift_is_reported(tree):
    ...
    pyproject.write_text(pyproject.read_text(encoding="utf-8").replace("netops-auditor==0.2.7", "netops-auditor"),
                         encoding="utf-8")
...
def test_sbom_version_drift_is_reported(component):
    ...
    sbom.write_text(sbom.read_text(encoding="utf-8").replace('"version": "0.2.2"', '"version": "0.0.9"', 1),
                    encoding="utf-8")
