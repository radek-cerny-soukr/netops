...
This source tree targets `netops-auditor/v0.2.8` (2026-09-26), which pins `netops-core==0.2.5` and is published on PyPI (`pip install netops-auditor`); every version keeps its own release page and tag, and every release stays in the history of `main` as its own signed commit - see [`docs/releasing.md`](https://github.com/radek-cerny-soukr/netops/blob/main/components/netops-auditor/docs/releasing.md). The component is usable from the CLI, and its shape is fixed by its gates rather than by its documentation.
