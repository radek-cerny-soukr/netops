...
Cross-component: [`verified-support.md`](verified-support.md) — per platform, what firmware, transport, authentication and account privilege the family has actually measured against a device, versus catalogue only; [`lab-measurements-2026-09-25.md`](lab-measurements-2026-09-25.md) — the combined 25 September 2026 catalogue and T-067 functional measurements: device preparation, per-query results, actual traffic, faults, recovery and coverage limits.
...
**All four components are also on PyPI**, from `netops-core` 0.2.4, `netops-auditor` 0.2.7, `netops-admin` 0.2.2 and `netops-helper` 0.3.7: a source distribution and a wheel built from the release tag by [`publish-pypi.yml`](../.github/workflows/publish-pypi.yml), uploaded through trusted publishing, first to TestPyPI. `pip install netops-auditor` or `pip install netops-admin` installs the exact pinned versions of the others. For `netops-helper` the package is the client side only: it installs the stdio proxy an MCP client launches. Its server runs as an image built from the release archive, and its runtime dependencies are installed there from a hash lock that `pip install` would not apply.
...
A component release archive is self-contained: it carries the component tree, its own `LICENSE`, and its own changelog. Self-contained does not mean identical in shape: the `netops-helper` archive additionally vendors `src/netops_core`, the `netops-auditor` archive pins `netops-core==0.2.5` and needs the core archive installed beside it, the `netops-admin` archive pins both, and the `netops-helper` archive pins `netops-core==0.2.5` and needs that core archive too, because its vendored copy of `netops_core` is there for the image build and does not satisfy the pin for `pip`, and `netops-core` is a library archive with no image of its own. Repository-level files (this page, `SECURITY.md`, `CONTRIBUTING.md`, CI) live here and are not part of a component archive.
